...
export class OPGateway extends EZCCIP {
	...
	constructor({provider1, provider2, L2OutputOracle, L2ToL1MessagePasser = '0x4200000000000000000000000000000000000016'}) {
		...
		this.register(`proveRequest(bytes context, tuple(bytes ops, bytes[] inputs)) returns (bytes)`, async ([index, {ops, inputs}], context, history) => {
			...
			return this.call_cache.get(hash, async () => {
				...
				let ctx = await prover.evalDecoded(ops, inputs);
				...
				return ABI_CODER.encode(
					...
					[[ethers.ZeroHash, output.stateRoot, output.passerRoot, output.blockHash], proofs, Uint8Array.from(order)]
				);
			});
		});
	}
