...
async function deploy_registry() {
	return foundry.deploy({sol: `
		contract Registry {
			struct Node { address registry; address resolver; }
			mapping (string => Node) _nodes;
			function setNode(string memory label, address registry, address resolver) external {
				_nodes[label] = Node(registry, resolver);
			}
		}
	`});
}
...
await foundry.confirm(root.setNode('eth', root_eth.target, ethers.ZeroAddress));
await foundry.confirm(root_eth.setNode('nick', root_eth_nick.target, nick_resolver.target));
await foundry.confirm(root_eth.setNode('leaf', ethers.ZeroAddress, leaf_resolver.target));
...
async function resolve(name) {
	...
	for (let label of namesplit(name)) {
		req.begin()
			.pushOutput(0).setTarget() // set registry
			//.requireContract() // not necessary as the read below will fail
			.push(0).setSlot() // _nodes mapping
			.pushStr(label).follow() // _nodes[label]
			.read() // registry
			//.debug()
			.begin()
	...
	req.eval({failure: true}); // loop until we get a failure
